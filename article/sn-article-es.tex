...
El conjunto de datos base fue Salony\footnote{Salony: User Story Dataset disponible en Hugging Face, \url{https://huggingface.co/datasets/salony/User_story}.}, del cual se seleccionaron 1,139 de 1,998 historias de usuario que cumplían con la estructura canónica \textit{“As a(n) [role], I want [feature] so that [benefit]”}. Cada historia fue procesada por dos modelos generadores (G1 y G2), configurados con diferentes parámetros de generación para fomentar comportamientos contrastantes. Las descomposiciones generadas fueron evaluadas mediante un modelo bajo el paradigma \textit{LLM-as-a-Judge}, asignando puntuaciones en cinco criterios: coherencia, completitud, viabilidad, formato y granularidad \cite{llm_judge_frontiers_2025}\cite{likert10granularity}\cite{likertscalecomparison}.
...
Cada criterio fue evaluado en una escala de 0 a 10, con una puntuación máxima de 50 puntos \cite{likert5to7range}\cite{dawes2008}. Se estableció un umbral de aprobación de 35 puntos (70\%), permitiendo clasificar automáticamente cada instancia como válida o no válida \cite{threshold70}.

Además de la puntuación total, se registraron métricas agregadas como media, desviación estándar, tasa de aprobación y tasa de victoria en comparaciones directas entre modelos. Este esquema permitió analizar el desempeño relativo de los generadores bajo distintas configuraciones.
...
En cada configuración, ambos generadores procesaron el mismo conjunto de historias de usuario, produciendo descomposiciones alternativas para cada instancia. Este filtrado permitió garantizar la consistencia en la entrada de los modelos y la validez de la evaluación. Posteriormente, el modelo evaluador analizó ambas salidas y asignó puntuaciones en los cinco criterios de calidad para determinar un ganador entre las dos alternativas. De esta forma se consigue no solo un análisis cuantitativo basado en puntuaciones, sino también una comparación directa entre modelos.
