...
y la metodología de evaluación. La Sección~\ref{sec5} presenta los resultados obtenidos.
La Sección~\ref{sec6} discute los hallazgos y sus implicaciones, mientras que la Sección~\ref{sec7}
...
%TODO: Más adelante mostrar el diseño de prompts con ejemplos reales. en 3.5 se habló de manera abstracta. Incluir al menos un fragmento del prompt de generación y uno del prompt del juez (en forma de figura o listado)
%
...
%TODO: Desarrollar la validación humana. Cuando se tenga, mencionar que evaluadores humanos revisaron una muestra representativa, especificar: por ejemplo, perfil de los evaluadores, métricas de acuerdo (e.g., Cohen's Kappa) y hallazgos.
...
\section{Resultados}\label{sec5}
% TODO: incluir tablas con estadísticos descriptivos por generador y por criterio.

Con el objetivo de evaluar el comportamiento del \textit{framework} propuesto bajo distintas configuraciones de modelos de lenguaje, se diseñaron cuatro pruebas experimentales independientes. Cada prueba se estructuró en torno a la comparación entre dos modelos generadores de descomposiciones de historias de usuario (G1 y G2), junto con un modelo evaluador que actuó bajo el paradigma \textit{LLM-as-a-Judge}.

En cada configuración, ambos generadores procesaron el mismo conjunto de historias de usuario, produciendo descomposiciones alternativas para cada instancia. Posteriormente, el modelo evaluador analizó ambas salidas y asignó puntuaciones en los cinco criterios de calidad: coherencia, completitud, viabilidad, formato y granularidad, conforme a la metodología descrita en la Sección~\ref{sec4}. Adicionalmente, el evaluador determinó un ganador entre las dos alternativas, permitiendo no solo un análisis cuantitativo basado en puntuaciones, sino también una comparación directa entre modelos.
...
Finalmente, los resultados se analizan tanto a nivel individual por prueba como de manera comparativa entre configuraciones, permitiendo evaluar patrones consistentes, posibles sesgos del modelo evaluador y diferencias entre evaluación cuantitativa y selección directa del mejor resultado.

\subsubsection{Test 1: Llama 3.1 8B vs Mistral 7B (Juez Qwen3 14B)}

Esta prueba compara el desempeño de los modelos llama3.1:8b (G1) y mistral:7b (G2) en la generación de descomposiciones de historias de usuario, utilizando qwen3:14b como modelo evaluador bajo el esquema \textit{LLM-as-a-Judge}.

\paragraph{Resultados agregados}

\begin{table}[h]
\centering
\caption{Resultados agregados del Test 1}
\begin{tabular}{lcc}
\toprule
Métrica & G1 (Llama 3.1 8B) & G2 (Mistral 7B) \\
\midrule
Puntuación promedio & 33.39 & 33.81 \\
Desviación estándar & 9.19 & 9.94 \\
Tasa de aprobación (\%) & 46.36 & 46.80 \\
Tasa de victoria (\%) & 74.45 & 25.55 \\
\bottomrule
\end{tabular}
\end{table}

Los resultados muestran que ambos modelos presentan un desempeño global muy similar en términos de puntuación promedio (33.39 vs 33.81) y tasa de aprobación (46.36\% vs 46.80\%). Asimismo, las desviaciones estándar relativamente altas (9.19 y 9.94) indican una variabilidad considerable en la calidad de las descomposiciones generadas, lo cual sugiere que ambos modelos producen resultados heterogéneos dependiendo de la historia de usuario procesada.

No obstante, se observa una diferencia significativa en la tasa de victoria, donde Llama 3.1 8B es seleccionado como mejor alternativa en el 74.45\% de los casos, a pesar de no presentar una ventaja clara en las métricas cuantitativas. Esta discrepancia evidencia una divergencia entre la evaluación basada en puntuaciones agregadas y la decisión comparativa del modelo evaluador.

\paragraph{Análisis por criterio}
% claridad, atomicidad, coherencia lógica, relevancia y completitud)
% coherencia, completitud, viabilidad, formato y granularidad

\begin{table}[h]
\centering
\caption{Puntuación promedio por criterio en el Test 1}
\begin{tabular}{lcc}
\toprule
Criterio & G1 (Llama 3.1 8B) & G2 (Mistral 7B) \\
\midrule
Completitud & 4.09 & 4.11 \\
Claridad & 4.08 & 4.15 \\
Accionabilidad & 3.88 & 4.07 \\
Estructura lógica & 4.12 & 3.92 \\
Granularidad & 3.61 & 3.84 \\
\bottomrule
\end{tabular}
\end{table}

El análisis por criterio permite profundizar en esta diferencia. Mistral 7B obtiene puntuaciones ligeramente superiores en completitud, claridad, accionabilidad y granularidad, lo que sugiere una tendencia hacia descomposiciones más completas, detalladas y orientadas a la ejecución. En contraste, Llama 3.1 8B presenta una ventaja en el criterio de estructura lógica (4.12 vs 3.92), indicando una mejor organización interna y coherencia estructural en las tareas generadas.

Estos resultados sugieren que el modelo evaluador podría estar asignando un mayor peso implícito a la estructura lógica de las descomposiciones al momento de seleccionar un ganador, incluso cuando otros criterios presentan valores inferiores. Este comportamiento es consistente con observaciones recientes en el paradigma \textit{LLM-as-a-Judge}, donde los modelos evaluadores no necesariamente siguen una agregación estrictamente lineal de los criterios definidos, sino que incorporan juicios globales basados en la calidad percibida de las salidas.

\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{images/barplot_criteria_test1.png}
\caption{Comparación de puntuaciones promedio por criterio entre ambos modelos en el Test 1.}
\label{fig:test1_criteria}
\end{figure}

La Figura~\ref{fig:test1_criteria} confirma visualmente que las diferencias entre modelos son relativamente pequeñas en la mayoría de los criterios, aunque se observa una ligera ventaja de Mistral 7B en aspectos relacionados con la claridad y la accionabilidad, mientras que Llama 3.1 8B destaca en estructura lógica.

\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{images/boxplot_scores_test1.png}
\caption{Distribución de puntuaciones totales para ambos modelos en el Test 1.}
\label{fig:test1_boxplot}
\end{figure}

Por otro lado, la Figura~\ref{fig:test1_boxplot} muestra que las distribuciones de puntuaciones de ambos modelos presentan un alto grado de solapamiento, lo que indica que sus desempeños son comparables en términos globales. No obstante, se observa una mayor dispersión en las puntuaciones de Mistral 7B, lo cual sugiere una mayor variabilidad en la calidad de sus salidas.

% \paragraph{Discusión de resultados}

Los resultados del Test 1 revelan una discrepancia clara entre las métricas cuantitativas agregadas y la decisión comparativa del modelo evaluador. Aunque ambos modelos presentan puntuaciones promedio y tasas de aprobación prácticamente equivalentes, la tasa de victoria favorece de manera significativa a Llama 3.1 8B.

Este comportamiento sugiere que el modelo evaluador no opera únicamente mediante una agregación lineal de los criterios definidos, sino que incorpora juicios comparativos globales al momento de seleccionar la mejor descomposición. En particular, el análisis por criterio indica que Llama 3.1 8B presenta una ventaja en estructura lógica, mientras que Mistral 7B obtiene mejores resultados en criterios como claridad, accionabilidad y granularidad.

La preferencia sistemática por Llama 3.1 8B sugiere la existencia de un peso implícito diferencial asignado a la estructura lógica dentro del proceso de evaluación, lo que indica que la coherencia organizativa de las tareas puede influir de manera desproporcionada en la decisión final del evaluador.

En conjunto, estos resultados evidencian que la evaluación basada en \textit{LLM-as-a-Judge} puede capturar dimensiones cualitativas que no se reflejan completamente en las métricas numéricas. Asimismo, resaltan la importancia de combinar análisis cuantitativos con comparaciones directas entre modelos, ya que ambos enfoques pueden conducir a interpretaciones diferentes sobre el desempeño de los sistemas evaluados.
...
%
% TODO: Considerar dentro de la discusión : discutir validez interna (el juez mide lo que dice medir?), validez externa (los resultados generalizan más allá del dataset?) y validez de constructo (los cinco criterios capturan suficientemente la calidad de una descomposición?)

\section{Conclusión}\label{sec7}
