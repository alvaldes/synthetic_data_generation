...
Este trabajo realiza dos contribuciones principales. En primer lugar, presenta un
...
\subsection{Modelo de Ejecución del Pipeline}
...
Desde una perspectiva de ingeniería, este modelo promueve un alto grado de desacoplamiento entre componentes. Cada paso encapsula su lógica de procesamiento y se comunica con el resto del pipeline exclusivamente a través del DataFrame compartido. Este tipo de diseño modular ha sido ampliamente adoptado en sistemas modernos de procesamiento de datos y aprendizaje automático, donde la composición de transformaciones reutilizables permite construir flujos de trabajo complejos de manera flexible \cite{dataflow_llm}.
...
Estos componentes conforman una arquitectura en la que cada etapa del procesamiento se encuentra claramente definida y desacoplada, desde la generación hasta la evaluación. Esta separación de responsabilidades permite no solo extender el pipeline mediante la incorporación de nuevos módulos, sino también experimentar con distintas configuraciones de modelos y estrategias de evaluación sin introducir dependencias rígidas, alineándose con principios fundamentales de diseño en arquitecturas orientadas a procesamiento de datos.
...
El estudio experimental se realizó utilizando el framework \textit{LocalLLM-DataForge}, con el objetivo de evaluar la generación automática de descomposiciones de historias de usuario mediante modelos de lenguaje ejecutados localmente.

El entorno consistió en un servidor Dell equipado con un procesador AMD EPYC 9124 a 3.0 GHz (16 núcleos, 32 hilos, 64 MB de caché), con 64 GB de memoria RAM DDR5-4800 (4 módulos de 16 GB) y una GPU NVIDIA Ampere A2 con 16 GB de memoria. El pipeline se ejecutó utilizando Python v3.13.7 y Ollama v0.21.2 como plataforma de orquestación para la ejecución local de los modelos. La GPU fue utilizada para acelerar la inferencia de los modelos, permitiendo reducir los tiempos de generación y evaluación durante las pruebas experimentales.
...
Las descomposiciones generadas fueron evaluadas mediante un modelo bajo el paradigma \textit{LLM-as-a-Judge}, asignando puntuaciones en cinco criterios: coherencia, completitud, viabilidad, formato y granularidad \cite{llm_judge_frontiers_2025}.
...
Cada criterio fue evaluado en una escala de 0 a 10, con una puntuación máxima de 50 puntos \cite{likert5to7range}. La puntuación total de cada instancia se obtuvo como la suma de los cinco criterios evaluados. A partir de estas puntuaciones, se estableció un umbral de aprobación de 35 puntos (70\%), permitiendo clasificar cada instancia como válida o no válida \cite{threshold70}. 

Sobre el conjunto de instancias evaluadas, se calcularon métricas agregadas como la media y la desviación estándar de las puntuaciones totales. La tasa de aprobación se definió como el porcentaje de instancias cuya puntuación total supera el umbral establecido. Por su parte, la tasa de victoria se calculó en el contexto de comparaciones directas entre dos descomposiciones, representando el porcentaje de comparaciones en las que una descomposición es preferida sobre la otra según la puntuación total asignada por el modelo evaluador.
...
La Tabla~\ref{tab:global_results} presenta las métricas agregadas para cada prueba, incluyendo la puntuación promedio junto con su desviación estándar (\textit{Mean} $\pm$ \textit{Std}), así como la tasa de aprobación (\textit{Pass}) y la tasa de victoria en comparaciones directas (\textit{Win}).
...
\toprule
Config. & Gen. & Mean $\pm$ Std & Pass (\%) & Win (\%) \\
\midrule
...
En contraste con las configuraciones anteriores, C4 presenta un comportamiento más equilibrado entre ambos generadores, con diferencias mínimas tanto en puntuación promedio como en tasa de victoria. Este resultado sugiere un escenario de convergencia en el desempeño, donde ninguno de los modelos domina claramente sobre el otro.
...
\caption{Distribución de puntuaciones totales por configuración, para generadores con temperatura baja (G1: T=0.3) y temperatura alta (G2: T=0.7).}
...
En el criterio de \textit{coherencia}, ambos modelos presentan desempeños similares en todas las configuraciones, con diferencias mínimas. En C2, ambos alcanzan valores altos y prácticamente equivalentes, mientras que en C3 se observa una ligera ventaja de G1. En C4, esta tendencia se mantiene, con puntuaciones muy cercanas entre ambos modelos, lo que refuerza la estabilidad de este criterio frente a cambios en la configuración.

En \textit{completitud}, se observa un comportamiento diferenciado. G2 supera a G1 en la configuración C2, alcanzando valores notablemente más altos. Sin embargo, en C3, G1 obtiene una puntuación considerablemente superior, lo que indica que el desempeño en este criterio depende fuertemente de la configuración del modelo. En C4, G2 recupera una ligera ventaja, aunque con diferencias menos pronunciadas, lo que sugiere un escenario más equilibrado entre ambos modelos.

En cuanto a la \textit{viabilidad}, ambos modelos presentan resultados cercanos en C1 y C2, con una ligera ventaja de G2 en C2. No obstante, en C3, G1 muestra un mejor desempeño, alcanzando valores más altos y consistentes. En C4, G1 mantiene una ventaja marginal, aunque las diferencias entre modelos son reducidas en comparación con configuraciones anteriores.

El criterio de \textit{formato} muestra uno de los comportamientos más estables entre modelos. En C1 y C2, ambos generadores presentan valores prácticamente idénticos, lo que sugiere que el cumplimiento de estructuras formales es relativamente independiente del modelo. Sin embargo, en C3, G1 supera claramente a G2. En C4, ambos modelos alcanzan nuevamente valores idénticos, reforzando la consistencia de este criterio a través de distintas configuraciones.

Finalmente, en \textit{granularidad}, se observan diferencias más marcadas. Aunque ambos modelos presentan valores similares en C1, en C2 G2 obtiene una ligera ventaja. No obstante, en C3, G1 supera significativamente a G2, alcanzando una mayor adecuación en el nivel de descomposición de las tareas. En C4, G2 vuelve a presentar una ligera ventaja, aunque en un contexto de menor separación entre modelos.
...
Este comportamiento es consistente con lo reportado en la literatura reciente sobre el uso de LLMs en ingeniería de requisitos. Estudios como \cite{zadenoori2025} señalan que el desempeño de los modelos en tareas de procesamiento de lenguaje natural depende fuertemente de factores como el diseño del prompt, la configuración de generación y el contexto de uso. Asimismo, Norheim et al. \cite{sami2024} identifican la sensibilidad a la configuración como uno de los principales desafíos al aplicar LLMs en tareas de ingeniería de requisitos, lo que coincide con las variaciones observadas entre las configuraciones C2 y C3 en este estudio.
...
En este contexto, la configuración C4 introduce un escenario particularmente relevante, donde ambos generadores presentan capacidades comparables en términos de tamaño del modelo. Bajo estas condiciones, las diferencias de desempeño se reducen considerablemente, lo que sugiere que parte de la variabilidad observada en otras configuraciones puede estar asociada no solo a parámetros como la temperatura, sino también a diferencias en capacidad y arquitectura entre modelos. 

Este resultado indica que, cuando se controlan factores como el tamaño del modelo, los sistemas tienden a converger en su desempeño, desplazando las diferencias hacia aspectos más finos como el estilo de generación o la priorización de ciertos criterios de calidad.

Un hallazgo relevante es la diferencia sistemática entre los generadores G1 y G2 en términos de estabilidad y variabilidad. Este fenómeno también ha sido reportado en estudios previos. En particular, Niv \cite{hematt2025} destaca que, aunque los LLMs pueden alcanzar altos niveles de desempeño promedio, su comportamiento puede presentar variaciones significativas entre ejecuciones, lo que impacta la reproducibilidad de los resultados. Mientras que G2 muestra en algunos casos puntuaciones promedio más altas (e.g., C2), sus resultados presentan mayor dispersión, lo que indica una menor consistencia en la calidad de las salidas. En contraste, G1 tiende a generar descomposiciones más estables, especialmente en configuraciones más complejas como C3.

Este comportamiento puede explicarse, en parte, por la diferencia en la temperatura utilizada durante la generación. Estudios han demostrado que la temperatura tiene un impacto directo en la estabilidad de las salidas. Donde valores más bajos tienden a favorecen salidas más deterministas y mejorar la consistencia, mientras que valores más altos introducen mayor diversidad e incrementan la variabilidad \cite{li2026temp}. Además, se ha observado que esta relación no es trivial y depende del tipo de tarea y del modelo utilizado, lo que refuerza la necesidad de ajustar cuidadosamente este parámetro en pipelines de generación y evaluación.
...
Este comportamiento es consistente con tendencias reportadas en la literatura reciente. En particular, Ferrari y Spoletini \cite{proci2025} señalan que, aunque los LLMs muestran un desempeño sólido en tareas con restricciones formales o estructurales, aún presentan limitaciones en aspectos relacionados con la comprensión semántica profunda, como la adecuación del contenido y la cobertura de requisitos. Estas limitaciones están asociadas a preocupaciones sobre la corrección y confiabilidad de los artefactos generados por LLMs en ingeniería de requisitos, lo que explica la mayor estabilidad observada en criterios como formato y coherencia, en contraste con la variabilidad en completitud y granularidad.
...
Este enfoque se encuentra alineado con una línea creciente de investigación que propone el uso de modelos como evaluadores automáticos. Estudios recientes han demostrado que estos sistemas pueden alcanzar niveles comparables con evaluadores humanos en diversas tareas \cite{eigler2026}. Sin embargo, también se han identificado limitaciones importantes, como la presencia de sesgos y la variabilidad en las puntuaciones asignadas. En particular, se ha señalado que estos sistemas pueden mostrar una tendencia a favorecer ciertos estilos de generación o estructuras de respuesta dependiendo del modelo evaluador y de su configuración. Esta observación resulta consistente con los resultados obtenidos en este estudio, donde las diferencias entre configuraciones evidencian que parámetros como la temperatura pueden influir no solo en la generación, sino también en la estabilidad de la evaluación automática.

En este contexto, enfoques como el propuesto por Do et al. \cite{daly2025} sugieren la incorporación de procesos iterativos y validación humana para mejorar la fiabilidad de los sistemas de evaluación automática, lo que representa una posible línea de mejora para el presente trabajo.

En relación con la generación de datos sintéticos, los resultados obtenidos son consistentes con trabajos como Li et al. \cite{long2024}, donde se señala que los LLMs pueden generar datos de alta calidad para tareas de procesamiento de lenguaje natural, aunque con limitaciones en términos de consistencia y cobertura. De manera similar, Ma et al.\cite{ma2025} y Chakir y Lahsen-Cherif \cite{fsidi2025} muestran que el uso de datos sintéticos puede mejorar el desempeño de modelos en dominios específicos, pero requiere mecanismos de validación robustos para garantizar su calidad. Estos hallazgos se reflejan en los resultados de este estudio, donde la calidad de las descomposiciones generadas depende significativamente de la configuración del modelo y del proceso de evaluación aplicado.

En términos de implicaciones, los resultados sugieren que el uso de LLMs locales constituye una alternativa prometedora para la generación de datasets sintéticos en ingeniería de requisitos, siempre que se integren mecanismos de validación estructurados dentro del pipeline. En particular, la combinación de generación controlada y evaluación automática permite mitigar la variabilidad inherente de los modelos y mejorar la consistencia de los artefactos generados. No obstante, los resultados también indican que la calidad alcanzada depende fuertemente de la configuración empleada, por lo que estos enfoques deben considerarse como herramientas de apoyo al proceso, más que como sustitutos directos de la validación humana.
...
Los resultados experimentales evidencian que es posible generar descomposiciones de tareas con niveles consistentes de calidad utilizando modelos de código abierto ejecutados localmente. Asimismo, se observó que el desempeño de los modelos depende significativamente de la configuración empleada, particularmente de la combinación de modelos y de parámetros como la temperatura. No obstante, también se identificaron escenarios donde las diferencias entre modelos se reducen considerablemente, evidenciando un comportamiento de convergencia cuando se utilizan modelos con capacidades similares. Este resultado sugiere que parte de la variabilidad observada no solo depende de parámetros de generación, sino también de factores como el tamaño y la arquitectura del modelo. En este contexto, el uso de estrategias de evaluación basadas en \textit{LLM-as-a-Judge} demostró ser útil para comparar salidas alternativas y capturar diferencias que no son evidentes mediante métricas agregadas.

El análisis por criterio permitió identificar que los modelos tienden a comportarse de manera más estable en dimensiones estructurales como formato y coherencia, mientras que presentan mayor variabilidad en aspectos semánticos como completitud y granularidad. Este comportamiento se mantiene incluso en escenarios donde el desempeño global entre modelos es similar, lo que sugiere que las diferencias se desplazan hacia dimensiones específicas de calidad más que al rendimiento global.
...
En relación con las contribuciones planteadas al inicio, este trabajo cumple dos aportaciones principales. En primer lugar, se presentó un framework completamente local, extensible y reproducible para la generación de datasets sintéticos a partir de historias de usuario. Este se basa en pipelines experimentales que permiten la orquestación controlada de procesos de generación y evaluación, reduciendo la dependencia de servicios en la nube. En segundo lugar, se definieron y operacionalizaron estrategias automáticas de validación basadas en \textit{LLM-as-a-Judge}. Estas demostraron ser efectivas para comparar descomposiciones y capturar diferencias no evidentes mediante métricas agregadas, aunque sujetas a variabilidad dependiendo de la configuración del modelo.
