...
class ValidateUserStories(BaseStep):
    ...
    def process(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        self.logger.info(
            f"Valid stories: {
                valid_count}/{total_stories} ({valid_count / total_stories * 100:.1f}%)"
        )
        self.logger.info(
            f"Invalid stories: {
                invalid_count}/{total_stories} ({invalid_count / total_stories * 100:.1f}%)"
        )

        # Log samples of invalid stories for debugging
        if invalid_count > 0:
            invalid_stories = df[~valid_mask][self.story_column].head(
                3).tolist()
