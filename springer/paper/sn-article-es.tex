...
To identify consistent patterns in the behavior of the validator models, a comparative analysis was conducted by aggregating the results of the experimental tests. This approach enables the observation of global trends regarding performance, stability, and relative behavior between models.
...
\begin{table}[ht]
\centering
\caption{Resultados agregados por prueba experimental.}
\label{tab:global_results}
\begin{tabular}{cllccc}
\toprule
Config. & Generador & Media $\pm$ Desv. Est. & Aprobado (\%) & Victoria (\%) \\
\midrule
...
In particular, low mean values observed in configurations such as C1 and C4 are strongly influenced by the occurrence of zero-score instances. These values are not random noise but correspond to cases where the generated decompositions were fully rejected by the validator model. As a result, the score distributions are highly skewed, with a substantial gap between mean and median values.

This effect is especially evident in C1, where both generators exhibit similar mean scores despite the boxplot in Figure~\ref{fig:global_boxplot} showing relatively high median values. This indicates that while models such as Llama 3.1 and Mistral are capable of producing high-quality outputs, they also frequently generate invalid decompositions that fail to meet the evaluator criteria.
...
\begin{figure}[ht]
\centering
\includegraphics[width=\columnwidth]{images/total_score_distribution_es.png}
...
These findings underscore the importance of considering both central tendency and variability when evaluating model performance. From an engineering standpoint, reliability becomes a critical factor: a model that occasionally produces high-quality outputs but frequently fails validation may be less desirable than one that consistently generates moderate but valid results.
...
\begin{table}[ht]
\centering
\caption{Puntuaciones promedio por criterio a través de las configuraciones y los generadores.}
\label{tab:global_criteria}
\begin{tabular}{clccccc}
\toprule
Config. & Generador & Coherencia & Completitud & Viabilidad & Formato & Granularidad \\
\midrule
...
\begin{figure}[ht]
\centering
\includegraphics[width=\columnwidth]{images/criterion_comparison_all_es.png}
...
It is important to note that these criterion-level averages are also influenced by the presence of zero-score instances discussed in the previous section. Fully rejected outputs contribute low values across all criteria, thereby affecting the overall averages and reinforcing the need to interpret these results in conjunction with distribution-based analyses.
...
Another relevant finding is the effectiveness of the \textit{LLM-as-a-Judge} approach. Using a validator model to compare alternative decompositions enables the identification of qualitative differences that are not captured by aggregate metrics alone. In this context, the win rate serves as a complementary indicator of relative preference.
...
The experimental results show that it is feasible to generate high-quality task decompositions using open-source models deployed locally. However, performance was found to be highly sensitive to configuration, including the combination of models and generation parameters such as temperature. This sensitivity manifests not only in average performance but also in variability, highlighting a trade-off between output quality and stability. In particular, scenarios involving models with comparable capacity show reduced performance gaps, suggesting that differences shift from overall quality to more subtle aspects such as consistency and generation behavior.

The analysis further revealed that model performance varies across evaluation dimensions. Structural criteria, such as \textit{format} and \textit{coherence}, exhibit stable behavior across configurations, whereas semantic criteria, including \textit{completeness} and \textit{granularity}, show greater variability. This indicates that while LLMs can reliably adhere to formal constraints, capturing deeper semantic aspects of decomposition remains more challenging and configuration-dependent.

Additionally, the \textit{LLM-as-a-Judge} strategy proved effective as a comparative evaluation mechanism, enabling the identification of qualitative differences not captured by aggregate metrics alone. Nevertheless, the results also highlight that automated validation is subject to variability and potential bias, reinforcing the need for careful configuration and complementary validation strategies.

Overall, this work demonstrates that local LLM-based pipelines constitute a viable and practical approach for synthetic dataset generation in requirements engineering. Beyond reducing dependency on external services, the proposed framework enables controlled experimentation and systematic evaluation, contributing to reproducibility in LLM-based research.

The main contributions of this work can be summarized as follows: (i) the design and implementation of a fully local and extensible framework for synthetic data generation from user stories; (ii) the integration of automated validation mechanisms based on \textit{LLM-as-a-Judge} within reproducible experimental pipelines; and (iii) an empirical analysis highlighting the impact of configuration on both performance and stability.

Future work will focus on improving evaluation reliability through hybrid approaches that combine automated and human validation, as well as exploring consensus mechanisms among multiple validator models to mitigate bias. Additionally, extending the dataset to more diverse and less structured domains will be essential to assess the generalizability of the approach. Finally, further research will investigate the impact of advanced prompting techniques and fine-tuning strategies, as well as the application of the framework to other requirements engineering artifacts such as use cases and functional specifications.
