...
class ValidateUserStories(BaseStep):
    ...
    def process(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Log statistics
        self.logger.info(
            f"Valid stories: {valid_count}/{total_stories} ({valid_count / total_stories * 100:.1f}%)"
        )
        self.logger.info(
            f"Invalid stories: {invalid_count}/{total_stories} ({invalid_count / total_stories * 100:.1f}%)"
        )
